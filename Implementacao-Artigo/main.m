...
% Huffman encode
[img_encoded, img_dict] = huffman_encode(img);
figure; imshow(img_compress); title('Após aplicação da codificação de Huffman');

%%=========================== Descompressão =============================%%

% Huffman decode
img_decoded = huffman_decode(img_encoded, img_dict);
figure; imshow(img_decompress); title('Após aplicação da decodificação de Huffman');

% Dequantização da imagem aplicada sobre cada bloco DCT
img_dequant = blockproc(img_quant, [8 8], @dct_dequantization);
